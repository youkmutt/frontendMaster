/* tslint:disable */
/* eslint-disable */
/**
 * File Upload API
 * No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)
 *
 * The version of the OpenAPI document: v1
 *
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import * as runtime from "../runtime";

export interface ApiFilesFileNameGetRequest {
  fileName: string;
}

export interface ApiFilesUploadPostRequest {
  file: Blob;
}

export interface ApiFilesV2FileNameGetRequest {
  fileName: string;
  filePath?: string;
}

export interface ApiFilesV2UploadPostRequest {
  file: Blob;
}

/**
 * FilesApi - interface
 *
 * @export
 * @interface FilesApiInterface
 */
export interface FilesApiInterface {
  /**
   *
   * @param {string} fileName
   * @param {*} [options] Override http request option.
   * @throws {RequiredError}
   * @memberof FilesApiInterface
   */
  apiFilesFileNameGetRaw(
    requestParameters: ApiFilesFileNameGetRequest,
    initOverrides?: RequestInit | runtime.InitOverrideFunction
  ): Promise<runtime.ApiResponse<void>>;

  /**
   */
  apiFilesFileNameGet(
    requestParameters: ApiFilesFileNameGetRequest,
    initOverrides?: RequestInit | runtime.InitOverrideFunction
  ): Promise<void>;

  /**
   *
   * @param {Blob} file
   * @param {*} [options] Override http request option.
   * @throws {RequiredError}
   * @memberof FilesApiInterface
   */
  apiFilesUploadPostRaw(
    requestParameters: ApiFilesUploadPostRequest,
    initOverrides?: RequestInit | runtime.InitOverrideFunction
  ): Promise<runtime.ApiResponse<void>>;

  /**
   */
  apiFilesUploadPost(
    requestParameters: ApiFilesUploadPostRequest,
    initOverrides?: RequestInit | runtime.InitOverrideFunction
  ): Promise<void>;

  /**
   *
   * @param {string} fileName
   * @param {string} [filePath]
   * @param {*} [options] Override http request option.
   * @throws {RequiredError}
   * @memberof FilesApiInterface
   */
  apiFilesV2FileNameGetRaw(
    requestParameters: ApiFilesV2FileNameGetRequest,
    initOverrides?: RequestInit | runtime.InitOverrideFunction
  ): Promise<runtime.ApiResponse<void>>;

  /**
   */
  apiFilesV2FileNameGet(
    requestParameters: ApiFilesV2FileNameGetRequest,
    initOverrides?: RequestInit | runtime.InitOverrideFunction
  ): Promise<void>;

  /**
   *
   * @param {Blob} file
   * @param {*} [options] Override http request option.
   * @throws {RequiredError}
   * @memberof FilesApiInterface
   */
  apiFilesV2UploadPostRaw(
    requestParameters: ApiFilesV2UploadPostRequest,
    initOverrides?: RequestInit | runtime.InitOverrideFunction
  ): Promise<runtime.ApiResponse<void>>;

  /**
   */
  apiFilesV2UploadPost(
    requestParameters: ApiFilesV2UploadPostRequest,
    initOverrides?: RequestInit | runtime.InitOverrideFunction
  ): Promise<void>;
}

/**
 *
 */
export class FilesApi extends runtime.BaseAPI implements FilesApiInterface {
  /**
   */
  async apiFilesFileNameGetRaw(
    requestParameters: ApiFilesFileNameGetRequest,
    initOverrides?: RequestInit | runtime.InitOverrideFunction
  ): Promise<runtime.ApiResponse<void>> {
    if (requestParameters["fileName"] == null) {
      throw new runtime.RequiredError(
        "fileName",
        'Required parameter "fileName" was null or undefined when calling apiFilesFileNameGet().'
      );
    }

    const queryParameters: any = {};

    const headerParameters: runtime.HTTPHeaders = {};

    const response = await this.request(
      {
        path: `/api/files/{fileName}`.replace(
          `{${"fileName"}}`,
          encodeURIComponent(String(requestParameters["fileName"]))
        ),
        method: "GET",
        headers: headerParameters,
        query: queryParameters,
      },
      initOverrides
    );

    return new runtime.VoidApiResponse(response);
  }

  /**
   */
  async apiFilesFileNameGet(
    requestParameters: ApiFilesFileNameGetRequest,
    initOverrides?: RequestInit | runtime.InitOverrideFunction
  ): Promise<void> {
    await this.apiFilesFileNameGetRaw(requestParameters, initOverrides);
  }

  /**
   */
  async apiFilesUploadPostRaw(
    requestParameters: ApiFilesUploadPostRequest,
    initOverrides?: RequestInit | runtime.InitOverrideFunction
  ): Promise<runtime.ApiResponse<void>> {
    if (requestParameters["file"] == null) {
      throw new runtime.RequiredError(
        "file",
        'Required parameter "file" was null or undefined when calling apiFilesUploadPost().'
      );
    }

    const queryParameters: any = {};

    const headerParameters: runtime.HTTPHeaders = {};

    const consumes: runtime.Consume[] = [
      { contentType: "multipart/form-data" },
    ];
    // @ts-ignore: canConsumeForm may be unused
    const canConsumeForm = runtime.canConsumeForm(consumes);

    let formParams: { append(param: string, value: any): any };
    let useForm = false;
    // use FormData to transmit files using content-type "multipart/form-data"
    useForm = canConsumeForm;
    if (useForm) {
      formParams = new FormData();
    } else {
      formParams = new URLSearchParams();
    }

    if (requestParameters["file"] != null) {
      formParams.append("File", requestParameters["file"] as any);
    }

    const response = await this.request(
      {
        path: `/api/files/upload`,
        method: "POST",
        headers: headerParameters,
        query: queryParameters,
        body: formParams,
      },
      initOverrides
    );

    return new runtime.VoidApiResponse(response);
  }

  /**
   */
  async apiFilesUploadPost(
    requestParameters: ApiFilesUploadPostRequest,
    initOverrides?: RequestInit | runtime.InitOverrideFunction
  ): Promise<void> {
    await this.apiFilesUploadPostRaw(requestParameters, initOverrides);
  }

  /**
   */
  async apiFilesV2FileNameGetRaw(
    requestParameters: ApiFilesV2FileNameGetRequest,
    initOverrides?: RequestInit | runtime.InitOverrideFunction
  ): Promise<runtime.ApiResponse<void>> {
    if (requestParameters["fileName"] == null) {
      throw new runtime.RequiredError(
        "fileName",
        'Required parameter "fileName" was null or undefined when calling apiFilesV2FileNameGet().'
      );
    }

    const queryParameters: any = {};

    if (requestParameters["filePath"] != null) {
      queryParameters["filePath"] = requestParameters["filePath"];
    }

    const headerParameters: runtime.HTTPHeaders = {};

    const response = await this.request(
      {
        path: `/api/files/v2/{fileName}`.replace(
          `{${"fileName"}}`,
          encodeURIComponent(String(requestParameters["fileName"]))
        ),
        method: "GET",
        headers: headerParameters,
        query: queryParameters,
      },
      initOverrides
    );

    return new runtime.VoidApiResponse(response);
  }

  /**
   */
  async apiFilesV2FileNameGet(
    requestParameters: ApiFilesV2FileNameGetRequest,
    initOverrides?: RequestInit | runtime.InitOverrideFunction
  ): Promise<void> {
    await this.apiFilesV2FileNameGetRaw(requestParameters, initOverrides);
  }

  /**
   */
  async apiFilesV2UploadPostRaw(
    requestParameters: ApiFilesV2UploadPostRequest,
    initOverrides?: RequestInit | runtime.InitOverrideFunction
  ): Promise<runtime.ApiResponse<void>> {
    if (requestParameters["file"] == null) {
      throw new runtime.RequiredError(
        "file",
        'Required parameter "file" was null or undefined when calling apiFilesV2UploadPost().'
      );
    }

    const queryParameters: any = {};

    const headerParameters: runtime.HTTPHeaders = {};

    const consumes: runtime.Consume[] = [
      { contentType: "multipart/form-data" },
    ];
    // @ts-ignore: canConsumeForm may be unused
    const canConsumeForm = runtime.canConsumeForm(consumes);

    let formParams: { append(param: string, value: any): any };
    let useForm = false;
    // use FormData to transmit files using content-type "multipart/form-data"
    useForm = canConsumeForm;
    if (useForm) {
      formParams = new FormData();
    } else {
      formParams = new URLSearchParams();
    }

    if (requestParameters["file"] != null) {
      formParams.append("File", requestParameters["file"] as any);
    }

    const response = await this.request(
      {
        path: `/api/files/v2/upload`,
        method: "POST",
        headers: headerParameters,
        query: queryParameters,
        body: formParams,
      },
      initOverrides
    );

    return new runtime.VoidApiResponse(response);
  }

  /**
   */
  async apiFilesV2UploadPost(
    requestParameters: ApiFilesV2UploadPostRequest,
    initOverrides?: RequestInit | runtime.InitOverrideFunction
  ): Promise<void> {
    await this.apiFilesV2UploadPostRaw(requestParameters, initOverrides);
  }
}
